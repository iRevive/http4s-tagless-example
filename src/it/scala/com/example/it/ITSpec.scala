...
import com.example.util.error.ErrorIdGen
import com.example.util.execution.EffConcurrentEffect
import com.example.util.logging.{Loggers, TraceId}
...
trait ITSpec
  ...
  protected type Eff[A] = com.example.util.execution.Eff[A]
  ...
  protected def withApplication[A](timeout: Duration = DefaultTimeout)(program: Application[Eff] => Eff[A]): Unit =
    EffectAssertion(timeout) {
      Loggers
        .consoleContextLoggerAsync(Level.Info)
